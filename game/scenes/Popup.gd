...
func show_popup(text, screenTime=6.0):
	popup_label.text = text
	if popup_label.percent_visible != 0:
		popup_tween.stop_all()
	popup_label.percent_visible = 1.0
	popup_tween.interpolate_property(popup_label, 'percent_visible', 1.0, 0, 1.0, Tween.TRANS_LINEAR, Tween.EASE_OUT, screenTime)
	popup_tween.start()
